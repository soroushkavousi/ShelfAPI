...
public class LogInWithEmailCommandHandler(SignInManager<User> signInManager, UserManager<User> userManager, TokenService tokenService)
    : IRequestHandler<LogInWithEmailCommand, Result<UserCredentialView>>
{
    public async Task<Result<UserCredentialView>> Handle(LogInWithEmailCommand request, CancellationToken cancellationToken)
    {
